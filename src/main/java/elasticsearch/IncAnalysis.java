...
public class IncAnalysis {
	...
	private void init() throws UnknownHostException {
		...
		client = new PreBuiltTransportClient(Settings.EMPTY)
				.addTransportAddress(new TransportAddress(InetAddress.getByName(IncAnalysis.HOST), IncAnalysis.PORT));
	...
	public static void main(String[] args) throws IOException {
		...
		IncAnalysis es = new IncAnalysis();
		es.init();

		QueryBuilder qbTime = QueryBuilders.rangeQuery("@timestamp").from(IncAnalysis.START).to(IncAnalysis.END);
					...
					for (SearchHit hit2 : hits4ID) {
						...
						if (match.find()) {
							...
							if (hits4IDRME.totalHits > 0) {
								...
								FileWriter fw = null;
								// String dirName = "IncAnalysis " + sdf.format(now) + " " + now.getTime();
								String dirName = "snow";
