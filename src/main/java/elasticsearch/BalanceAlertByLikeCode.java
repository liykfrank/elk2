...
public class BalanceAlertByLikeCode {



	public final static String HOST = "10.140.8.212";
	public final static int PORT = 9300;
	public final static String START = "2018-07-31T15:59:59.000Z";
	...
	private void init() throws UnknownHostException {
		...
		client = new PreBuiltTransportClient(Settings.EMPTY).addTransportAddress(
				new TransportAddress(InetAddress.getByName(BalanceAlertByLikeCode.HOST), BalanceAlertByLikeCode.PORT));
	...
	public static void main(String[] args) throws IOException {
		...
		BalanceAlertByLikeCode es = new BalanceAlertByLikeCode();
		es.init();

		// CEST +02:00 中欧夏令时
		QueryBuilder qbTime = QueryBuilders.rangeQuery("@timestamp").from(BalanceAlertByLikeCode.START).to(BalanceAlertByLikeCode.END);
		...
		String regx2 = "eventTime(.*?)eventType";
		...
		// find all alert
		 for(int i = 0; i <= pageNum; i++) {
			for (SearchHit hit :  response4INC.getHits()) {
				...
				if (match.find()) {
					String time = match.group(1).substring(6, match.group(1).length() - 9);
					fw.write(time);
					fw.write("\r\n");
				} else {
